...
- **villa-api verification rule (added 2026-08-27, to stop repeated documentation mistakes):** `villa-api`'s actual behavior (routes, auth, integrations) must be verified directly via Supabase MCP (`list_edge_functions` / `get_edge_function` on project `svcmybsziaelwwdrnzcv`, slug `villa-api`) before being written into project-memory as fact. Do not infer or re-state villa-api behavior from this repo's client call sites, from older project-memory text, or from chat history — those have previously been wrong (e.g. "no server-side auth exists" and "WhatsApp goes through WhaCenter" were both incorrect until the source was actually read). If Supabase MCP access isn't available in a session, mark the claim UNKNOWN rather than repeating an old inference.
