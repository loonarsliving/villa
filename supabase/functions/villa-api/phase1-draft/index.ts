// PHASE 1 DRAFT — proposed v26. NOT DEPLOYED.
// Full file provided (rather than a patch-only diff) so it can be
// reviewed and test-compiled as a whole. See CHANGES.md in this same
// directory for a function-by-function explanation of every change
// against the live v25 (supabase/functions/villa-api/index.ts).
// Every endpoint not called out in CHANGES.md is unchanged from v25.
...
// CHANGE (Phase 1 §6.6): constant-time signature comparison, was `!==`.
...
// CHANGE (Phase 1 §6.6): live role/active-state revalidation on every
// authenticated request, instead of trusting the 7-day-TTL token's
// embedded role/unit_id unconditionally.
...
// NEW (Phase 1 §6.5): shared date-string validation. A malformed date
// string previously flowed straight into `new Date(...)` inside
// datesOverlap(), producing an Invalid Date whose comparisons silently
// evaluate to false -- which could defeat the double-booking check
// entirely for a bad payload rather than erroring.
...
// UNCHANGED from v25 -- frozen per docs/revenue-engine/PHASE0-BASELINE.md
// §2. Do not modify this function without a separate, explicit,
// owner-approved change (see that document's "Frozen financial
// baseline" section).
...
Deno.serve(async (req)=>{
  ...
  // CHANGE (Phase 1 §6.4, §6.5): date validation + server-computed price
  // for new bookings (this endpoint is only ever called for walk-in/
  // direct bookings -- Cloudbeds-sourced bookings are written directly to
  // `bookings` by the separate Next.js webhook route, never through
  // here). Client-supplied tarif/total_bayar are no longer trusted.
  ...
  // CHANGE (Phase 1 §6.2): now delegates to the atomic
  // villa_commit_checkin() RPC (see supabase/migrations/
  // 20260904000002_atomic_checkin_checkout_rpc.sql) instead of 3
  // sequential, unchecked writes. Duplicate/invalid-state check-in now
  // returns a clean 409 instead of silently re-running.
  ...
  // CHANGE (Phase 1 §6.3): now delegates to villa_commit_checkout().
