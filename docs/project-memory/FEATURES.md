...
- **Payment Gateway — walk-in cashier (Villa, Cafe, Spa)** — DONE, added 2026-08-23. `admin/payment-gateway/page.tsx`. Extra email-whitelist gate on top of the `admin` role (system has no separate "super admin" role); walk-in guest data entry → static-QRIS display with nominal overlay → manual lunas/batal marking. Cafe/Spa/Lainnya: backed by `villa-api` endpoints `GET/POST/PATCH /admin/walkin-payments` and a new `walkin_payments` table (see DATABASE.md) — not folded into investor revenue-sharing reports by design. Villa (for guests who book directly instead of via OTA/Cloudbeds): reuses the real booking pipeline — `POST /bookings` on QRIS creation, `POST /checkin` on "Tandai Lunas" (creates the `transactions` income row that DOES feed the 70/30 investor split, same as the Front Desk check-in flow), `PATCH /bookings` (new, villa-api v16) to cancel if unpaid. A "Booking Villa Walk-in Terbaru" list on the same page shows these bookings; they also appear in the existing Front Desk → Daftar Booking page. The QRIS image is stored server-side in `integration_settings` (key `walkin_qris`, via the existing `/admin/settings` endpoints), not in localStorage.
- **Investor visibility into cafe/spa income** — DONE, added 2026-08-23. `investor/page.tsx` shows a "Pemasukan Cafe & Spa (Walk-in)" card sourced from `GET /report`'s new `walkin_income` field — informational only (villa-wide, not per-unit, not part of the 70/30 split shown above it), added at the owner's explicit request since that income is expected to eventually be shared with investors too.
