...
- The frontend calls a **Supabase Edge Function** named `villa-api` at a hardcoded URL (`https://svcmybsziaelwwdrnzcv.supabase.co/functions/v1/villa-api`, see `src/lib/api.ts`) for essentially all business logic (login, CRUD for bookings/units/reports/users/etc). **The source lives at `supabase/functions/villa-api/index.ts` in this repo as a manually-synced snapshot** (not an auto-deployed one — see that directory's README for the sync/deploy procedure) — re-synced 2026-09-10 to the then-live v34 after being found stale at v26. Always verify against `mcp__Supabase__get_edge_function` (project `svcmybsziaelwwdrnzcv`, slug `villa-api`) before trusting the committed file for anything version-sensitive; it is not deployed from CI, so nothing enforces it staying current with the live function.
- **Relationship to "Mkhsistem" CONFIRMED 2026-08-27** (via direct `villa-api` source read + `integration_settings` query, Supabase MCP): Villa is only the frontend + one webhook route in this repo, plus `villa-api` (Supabase Edge Function) for everything else — and `villa-api` itself borrows two capabilities from the separate Mkhsistem (MK Connect) system at `https://mkh.haluoleo.id` rather than implementing them: WhatsApp sending (`sendWa()` proxies to Mkhsistem's `/api/wa/send`, confirmed live via `integration_settings.vercel_bridge.base_url`) and, on the still-unmerged CCTV branch, Gemini AI vision calls. Villa and Mkhsistem are separate repos/codebases; Villa does not own AI or WA infrastructure, it calls into Mkhsistem's. See ARCHITECTURE.md/INTEGRATIONS.md for detail.
