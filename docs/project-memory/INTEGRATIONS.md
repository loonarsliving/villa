...
- **Outbound reservation push added 2026-09-10 (villa → Cloudbeds, `villa-api`)**: `villa-api`'s `POST /bookings` now pushes any non-`cloudbeds`-sourced booking (walk-in/direct, created by staff at Front Desk) to Cloudbeds via `POST /postReservation`, so the room shows blocked in Cloudbeds/OTAs too — closing the loop that was previously one-way (Cloudbeds→villa only). Room type is resolved live via `GET /getRooms` matched against `cloudbeds_room_mapping`; the reservation source defaults to the first non-third-party active source from `GET /getSources`, overridable via `integration_settings.cloudbeds_outbound.source_id`/`.guest_country_default`. Verified against Cloudbeds' published OpenAPI spec (`github.com/cloudbeds/openapi-specs`, `pms-v1.2`), not guessed. Best-effort/fail-soft like the WA bridge — a push failure never blocks or rolls back the villa booking, it's logged to `cloudbeds_events_log` with `event_type='outbound.reservation.created'`. Needs its own `CLOUDBEDS_API_KEY` set as a **Supabase Edge Function secret** on `villa-api` (separate from the same-named Vercel env var used by the frontend's read-only room picker) — until that secret exists the push is a silent no-op (documented, not a bug). **Known gap**: booking *cancellation* in villa does not yet push a cancellation to Cloudbeds — a villa-cancelled walk-in currently just leaves the room appearing blocked in Cloudbeds until someone cancels it there manually.
...
## EZVIZ CCTV live-view (added 2026-08-28)
- **Purpose**: watch villa's CCTV cameras (EZVIZ) live, from inside the admin interface. Not the same as the older unmerged AI-detection branch (`claude/frigate-ai-cctv-module-eqwuri`, periodic snapshot + Gemini presence detection) — this is real-time viewing only, admin-gated, built independently per owner's explicit scope choice.
- **Contract, verified before building** (`open.ys7.com`'s own docs are network-egress-blocked from this environment; verified via WebSearch + the SDK's own GitHub source instead): EZVIZ's Open Platform issues an account-level access token (`POST {EZVIZ_API_BASE}/api/lapp/token/get`, form-encoded `appKey`/`appSecret` → `{accessToken, expireTime}`). Live video is embedded via EZVIZ's own official web SDK, npm package `ezuikit-js` (`EZUIKitPlayer`) — not a plain `<video>` tag, since the stream isn't playable without their player resolving/decoding it. Player URL format: `ezopen://[verificationCode@]open.ys7.com/{deviceSerial}/{channelNo}.live` (verification code only needed for encrypted streams, embedded as URL userinfo).
- **Where secrets live**: `EZVIZ_APP_KEY`/`EZVIZ_APP_SECRET` are Vercel env vars only (same pattern as `CLOUDBEDS_API_KEY`), read by `src/lib/ezviz.ts` (server-only, caches the access token in-process). **Tradeoff inherent to EZVIZ's own SDK design**: the account-level access token itself IS sent to the browser (EZUIKitPlayer takes it as a constructor param) — there's no way to use their official player without this. Mitigated by gating the token-issuing route (`/api/cctv/token`, admin-only via the existing `isAdminToken` helper) so only authenticated admin sessions can request it; `EZVIZ_APP_KEY`/`EZVIZ_APP_SECRET` themselves never reach the browser.
- **Camera registry**: `cctv_cameras` table (see DATABASE.md), managed via villa-api v23 `/admin/cctv/cameras`.
...
## Pembayaran QRIS — STATIS, tidak ada payment gateway (keputusan owner 2026-09-19)
- **Purpose**: menerima pembayaran tamu (villa walk-in, cafe, spa) lewat QRIS statis milik villa sendiri.
- **Location in code**: `src/lib/walkin.ts` (`loadQrisImage`/`saveQrisImage`/`clearQrisImage` → villa-api `/walkin-qris`); `src/app/front-desk/payment-gateway/page.tsx` (unggah gambar/PDF QRIS + tampilkan ke tamu); `src/lib/pdfToImage.ts` (konversi lembar QRIS PDF jadi PNG di browser).
- **Data IN/OUT**: satu gambar QRIS (data URL) disimpan di `integration_settings.walkin_qris.data_url`. Tidak ada panggilan keluar ke pihak mana pun.
- **Authentication**: tidak ada — tidak ada pihak ketiga yang dihubungi.
- **Status**: ACTIVE. Gambar QRIS sudah terisi per 2026-09-19.
- **Konsekuensi yang melekat pada QRIS statis, bukan bug**: kodenya tidak membawa nominal (tamu mengetik sendiri jumlahnya) dan tidak ada konfirmasi otomatis (tidak ada webhook, tidak ada cek status). Klik "Tandai Lunas" oleh kasir adalah catatan pembayarannya; untuk villa klik itu sekaligus menjalankan check-in. Layar kasir menuliskan kedua hal ini terang-terangan supaya tidak ada yang mengira ada pengaman otomatis.

### iPaymu — DIHAPUS 2026-09-19
Integrasi QRIS dinamis iPaymu (ditambahkan 2026-08-27) dihapus atas keputusan
owner: *"Saya tidak pakai ipaymu saya pakai qris statis"*. Yang dihapus:
`src/lib/ipaymuApi.ts`, `/api/payment-gateway/qris`, `/api/payment-gateway/qris/status`,
`/api/webhooks/ipaymu`, dan dependensi `qrcode`. Kredensialnya memang tidak
pernah ada di Vercel, jadi tidak ada satu pun transaksi sungguhan yang pernah
melewatinya (`walkin_payments` kosong saat dihapus). Kodenya tetap ada di
riwayat git kalau suatu saat dibutuhkan lagi.
