...
/**
 * Vercel Cron target (see vercel.json, tanggal 25 tiap bulan, `0 12 25 * *` UTC = 19:00 WIB --
 * 1 jam sebelum batas transfer dividen jam 20:00 WIB). Same CRON_SECRET guard as the other
 * cron routes; calls villa-api's own POST /cron/dividend-transfer-reminder (guarded
 * separately by integration_settings.cron.secret) so the dividend computation + WhatsApp
 * send logic stays in one place, same pattern as /api/cron/dividend-list.
 */
export async function GET(request: Request) {
  ...
  try {
    ...
    const res = await fetch(`${API_BASE}/cron/dividend-transfer-reminder`, {
      method: "POST",
